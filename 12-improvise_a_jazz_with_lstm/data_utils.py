...
def generate_music(inference_model, corpus=corpus, abstract_grammars=abstract_grammars, tones=tones, tones_indices=tones_indices, indices_tones=indices_tones, T_y=10, max_tries=1000, diversity=0.5):
    ...
    # variable used to write sounds to the Stream.
    curr_offset = 0.0
    # number of different set of chords
    ...
    for i in range(1, num_chords):
        ...
        for j in chords[i]:
            # Add chord to the current chords with the adequate offset, no need
            # to understand this
        ...
        predicted_tones = predicted_tones.replace(
            ' A', ' C').replace(' X', ' C')
